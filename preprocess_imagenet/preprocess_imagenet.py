...
###############################################################################
# List of changes:
# - changed description below
# - removed the gcs part
# - changed name from imagenet_to_gcs.py to preprocess_imagenet.py

r"""Script to process the Imagenet dataset.
To run the script setup a virtualenv with the following library installed:
- `tensorflow-cpu`: Install with `pip install tensorflow-cpu`
Once you have the setup, you should register on the
...
python3 preprocess_imagenet.py \
  ...
  --local_scratch_dir="path/to/imagenet/tf_records"
```
"""
