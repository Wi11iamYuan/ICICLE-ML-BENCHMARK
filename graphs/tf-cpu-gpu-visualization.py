...
cpu_data = pd.read_csv('./graphs/tf-cpu-times.csv', header=None, names=['cores', 'time'])
cpu_data = cpu_data.iloc[1:]  # Remove the first row (number of datapoints)

gpu_data = pd.read_csv('./graphs/tf-gpu-times.csv', header=None, names=['time'])
...
plt.title('Tensorflow Benchmark: CPU vs GPU Performance')
